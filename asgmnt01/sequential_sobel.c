...
void compute_energy(const unsigned char *image, int width, int height, int *energy) {
    ...
    for (int i = 0; i < width * height; i++) {
        int idx = i * COLOR_CHANNELS;
        gray[i] = (image[idx] + image[idx + 1] + image[idx + 2]) / 3;
    }
        ...
        for (int x = 0; x < width; x++) {
            ...
            for (int i = -1; i <= 1; i++) {
                for (int j = -1; j <= 1; j++) {
                    ...
                    int gray_val = gray[yi * width + xi];
                    gx += gray_val * sobel_x[i + 1][j + 1];
                    gy += gray_val * sobel_y[i + 1][j + 1];
                }
            }
...
void log_execution_time(const char *filename, double elapsed_time) {
    ...
    char filepath[256];
    snprintf(filepath, sizeof(filepath), "sequential_results/sobel_%s.txt", filename);
